...
exports.deleteAllProperties = async (req, res) => {
  ...
  try {
    ...
    if (user) {
      try {
        const deleteProperty = await PropertyModel.deleteMany(userID);
        if (deleteProperty.deletedCount) {
          res.status(200).json({
            status: "Success",
            message: `Properties deleted successfully of account with USER ID: ${userID}`,
          });
        } else {
          res.status(400).json({
            status: "Failed",
            message: "Can't able to delete the properties!",
          });
        }
      } catch (error) {
        res.status(400).json({
